/*
 * Reduced rcore.h from bnlearn version 4.7 to avoid 
 * extensive warnings when compiled in bnsl.cpp.
 */
...
#define NLEVELS(x) Rf_length(Rf_getAttrib(x, R_LevelsSymbol))
#define NLEVELS2(data, j) \
length(Rf_getAttrib(VECTOR_ELT(data, j), R_LevelsSymbol))
